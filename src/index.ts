export type ColorValue =
  ...
  | [
      ...
      (
        | 'rgb'
        | 'hsl'
        | ((prefixKey: string, value: string) => string)
        | undefined
      ),
    ]
...
export function presetTheme(theme: DeepPartial<Theme>, options: Options) {
  ...
  return {
    ...
    plugins: [processPlugin(mergedTheme), ...(options.plugins || [])],
  }
}

export function generateColors(
  theme: Theme,
  options: Options = { colorRule: 'hsl' },
) {
  // 测试环境
        ...
        if (typeof colorValue === 'object' && !Array.isArray(colorValue)) {
          ...
          if (cssVarKey === 'DEFAULT') {
            ...
            if (typeof colorValue === 'string') {
              ...
              if (colorValue[1] === undefined) {
                colors[key][cssVarKey] =
                  `var(--${colorKey}, ${resolvedValue(colorValue[0])})`
              } else if (typeof colorValue[1] === 'string') {
                colors[key][cssVarKey] =
                  `${colorValue[1]}(var(--${colorKey}, ${resolvedValue(colorValue[0])}))`
              } else if (typeof colorValue[1] === 'function') {
                colors[key][cssVarKey] = colorValue[1](
                  `--${colorKey}`,
                  colorValue[0],
                )
              }
      ...
      if (typeof value === 'string' || Array.isArray(value)) {
        ...
        if (Array.isArray(value)) {
          ...
          if (typeof secondParam === 'string') {
            ...
            colorValue[key] =
              `var(${prefixKey}${key === 'DEFAULT' ? '' : `-${key}`}, ${colorString})`
            continue
          }
        ...
        const currentPrefixKey =
          key === 'DEFAULT' ? prefixKey : `${prefixKey}-${key}`
        ...
        if (isCompleteColor) {
          ...
          colorValue[key] =
            `${colorRule}(var(${currentPrefixKey}, ${colorString}))`
        }
 ...
 */
function resolveNestedColorFunctions(
  value: string,
  visited: string[],
  ...
  return value.replace(colorFuncRegex, (match, funcName, content) => {
    // 解析内容中的变量引用
    const { resolvedContent, finalVisited } = resolveVariablesInContent(
      content,
      visited,
      visitedSet,
    )
    ...
    if (nestedMatch) {
      ...
      // 保留透明度等修饰符
      const remainingContent = resolvedContent.replace(
        nestedFuncRegex,
        innerContent,
      )
...
function resolveVariablesInContent(
  ...
  const resolvedContent = content.replace(varRegex, (match, varName) => {
    ...
    if (colorFuncMatch) {
      ...
      // 递归解析内部内容
      const {
        resolvedContent: resolvedInnerContent,
      ...
      // 如果不是颜色函数，递归解析变量值
      const resolvedVarValue = resolveNestedColorFunctions(
        varValue,
        newVisited,
        newVisitedSet,
      )
